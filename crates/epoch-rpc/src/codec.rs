//! Data-plane protocol opcodes ([`Pcode`]) and the hand-written little-endian
//! codecs for their control structs (02 §5).
//!
//! A frame's payload is *either* a fixed control struct *or* a raw data body,
//! never both:
//! - control frames (`CreateExtent`, `CreateExtentResp`, `Open`, `End`,
//!   `ReadShard`, `Error`) carry exactly the struct bytes below;
//! - `Data` and a `ReadShardResp` *hit* carry a raw body (a bitrot frame /
//!   shard bytes) and no control struct, so the body stays a zero-copy slice of
//!   the received payload;
//! - `CommitAck` and a `ReadShardResp` *miss* carry an empty payload.
//!
//! A `ReadShardResp` distinguishes hit from miss with the header
//! [`FLAG_READ_HIT`] bit rather than an in-band byte, keeping the hit body a
//! clean whole-payload slice.
//!
//! Scalars are little-endian to match the on-disk record codecs
//! (`epoch-store`); `ExtentId` is an opaque 16-byte identity copied verbatim.
//!
